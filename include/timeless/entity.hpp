...
inline Entity create_entity(const std::string &debug_name = "") {
  ...
  if (entities >= MAX_ENTITIES) {
    if (out_of_ids) {
      throw std::runtime_error(
          "ERROR: Cannot create more entities, maximum limit reached.");
    }
  ...
  if (active_entities.contains(id)) {
    if (debug_name != "")
      std::cout << "Failed Creating entity: " << debug_name << " " << id
                << std::endl;
