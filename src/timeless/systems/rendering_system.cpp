...
void RenderingSystem::update_transform(std::shared_ptr<Transform> transform,
    ...
    transform->model = glm::translate(transform->model,
                                      transform->get_position_minus_offset());
    ...
    if (!transform->isometric) {
        if (transform->center) {
            ...
            if (transform->width != 0.0f && transform->height != 0.0f)
                transform->model = glm::translate(
                    transform->model,
                    glm::vec3(0.5 * transform->width, 0.0, 0.0));
        }
    }
    if (o != glm::vec3(0.0f))
    ...
    if (transform->isometric)
        transform->model = glm::rotate(transform->model, glm::radians(45.0f),
                                        glm::vec3(0, 0, 1));
...
void RenderingSystem::set_shader_transform_uniforms(
    ...
    if (shader != nullptr && transform != nullptr) {
        ...
        glUniformMatrix4fv(glGetUniformLocation(shader->ID, "model"), 1,
                           GL_FALSE, glm::value_ptr(transform->model));
...
void RenderingSystem::set_shader_sprite_uniforms(
    std::shared_ptr<Shader> shader, std::shared_ptr<Sprite> sprite, int tick,
...
void RenderingSystem::calculate_lighting(Entity /*ent*/,
                                          ComponentManager & /*cm*/,
        ...
        if (animation != nullptr && animation->playing) {
            ...
            for (const auto &bone : animation->bones) {
                ...
                if (bone->transform) {
                    ...
                    set_shader_transform_uniforms(bone_shader, bone->transform,
                                                  cam, x, y, zoom, tick);
                }
                if (auto spriteBone =
            ...
            if (font != nullptr) {
                if (!text->hidden) {
                    if (!animation || (animation && !animation->playing))
                        set_shader_transform_uniforms(shader, transform, cam, x,
                                                      y, zoom, tick);
                    text->render(font, 0.0f, 0.0f,
                                 get_text_font_height(entity, cm), shader);
                }
            }
        }
        ...
        if (animation == nullptr || !animation->playing) {
            ...
            if (sprite != nullptr) {
                if (!sprite->hidden) {
                    ...
                    if (sprite->slice_shader != nullptr) {
                        ...
                        auto texture = cm.get_component<Texture>(entity);
                        if (texture != nullptr) {
                            set_shader_transform_uniforms(shader, transform, cam,
                                                          x, y, zoom, tick);
                            ...
                            std::cout << "No texture for sprite entity "
                                      << entity << std::endl;
                        }
                    }
                }
            ...
            if (model != nullptr) {
                ...
                if (is_in_frustum(cam->get_frustum(x, y, zoom), sphere)) {
                    set_shader_transform_uniforms(shader, transform, cam, x, y,
                                                  zoom, tick);
                    ...
                    if (boneMatrixLoc >= 0) {
                        ...
                        if (skeletal_animation != nullptr) {
                            ...
                            size_t count =
                                skeletal_animation->poseMatrices.size();
...
bool RenderingSystem::is_in_frustum(const Frustum &frustum,
    ...
    for (int i = 0; i < 6; ++i) {
        ...
        float distance =
            glm::dot(glm::vec3(plane), sphere.center) + plane.w;
...
void RenderingSystem::prepare_instanced(ComponentManager &cm, float x, float y,
    ...
    if (!sprite_indices.empty())
        glBufferSubData(GL_ARRAY_BUFFER, 0,
                        sprite_indices.size() * sizeof(float),
                        sprite_indices.data());
...
void RenderingSystem::init_instanced_model_buffers(
    ...
    for (auto &mesh : model->meshes) {
        ...
        glBufferData(GL_ARRAY_BUFFER, max_instances * sizeof(glm::mat4),
        ...
        if (attrLoc2 >= 0) {
            glEnableVertexAttribArray(attrLoc2);
            glVertexAttribPointer(attrLoc2, 1, GL_FLOAT, GL_FALSE,
                                  sizeof(float), (void *)0);
            glVertexAttribDivisor(attrLoc2, 1);
        }
        ...
        glBufferData(GL_ARRAY_BUFFER, max_instances * sizeof(glm::vec4),
                     nullptr, GL_DYNAMIC_DRAW);
        GLint attrLoc3 = glGetAttribLocation(shader->ID, "aModelParams");
        if (attrLoc3 >= 0) {
            glEnableVertexAttribArray(attrLoc3);
            glVertexAttribPointer(attrLoc3, 4, GL_FLOAT, GL_FALSE,
...
void RenderingSystem::prepare_instanced_models(ComponentManager &cm,
    ...
    if (!modelIndices.empty())
        glBufferSubData(GL_ARRAY_BUFFER, 0,
                        modelIndices.size() * sizeof(float),
                        modelIndices.data());
...
void RenderingSystem::instanced_model_render(ComponentManager &cm, int x,
                                              int y, float zoom, int tick) {
    std::shared_ptr<Camera> cam = cm.get_component<Camera>(camera);
    if (cam != nullptr)
        pre_filter_lights(cam);
    ...
    for (auto &[modelPtr, entities] : modelGroups) {
        ...
        if (shader) {
            ...
            glUniform1f(glGetUniformLocation(shader->ID, "time"),
                        glfwGetTime());
            glUniform1f(glGetUniformLocation(shader->ID, "tick"), tick);
            ...
            for (auto &mesh : modelPtr->meshes) {
                ...
                {
                    GLint loc =
                        glGetAttribLocation(shader->ID, "aModelIndex");
                    if (loc >= 0) {
                        glEnableVertexAttribArray(loc);
                ...
                {
                    GLint loc =
                        glGetAttribLocation(shader->ID, "aModelParams");
                    if (loc >= 0) {
                        glEnableVertexAttribArray(loc);
                        glVertexAttribPointer(loc, 4, GL_FLOAT, GL_FALSE,
                                              sizeof(glm::vec4), (void *)0);
                ...
                glUniform3fv(
                    glGetUniformLocation(shader->ID, "materialDiffuse"), 1,
                ...
                glUniform1f(glGetUniformLocation(shader->ID, "roughness"),
                            0.1f);
                glUniform3fv(
                    glGetUniformLocation(shader->ID, "materialSpecular"), 1,
                    glm::value_ptr(mesh->specularColor));
                glUniform3fv(
                    glGetUniformLocation(shader->ID, "lightColor"), 1,
