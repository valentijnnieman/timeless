...
Sprite::Sprite(int index, glm::vec4 color, glm::vec2 spriteSheetSize,
               glm::vec2 spriteSize, bool isStatic,
               bool animating, int frames)
...
void Sprite::setColor(glm::vec4 newColor) { color = newColor; }
void Sprite::set_index(int newIndex) {
  index = newIndex;
  og_index = newIndex;
}
void Sprite::set_slice_shader(std::shared_ptr<Shader> shader,
  ...
  // if (width == 0) std::cerr << "Sprite framebuffer texture not valid!" <<
  // std::endl;
...
void Sprite::set_shader_sprite_uniforms(int textureUnit) {
  float cols = spriteSheetSize.x / spriteSize.x;

  float col = index - cols * floor(index / cols);
  float row = floor(index / cols);

  glUniform1i(glGetUniformLocation(slice_shader->ID, "texture1"), textureUnit);
...
void Sprite::set_shader_transform_uniforms(int tick) {
  glUniformMatrix4fv(glGetUniformLocation(slice_shader->ID, "projection"), 1,
                     GL_FALSE,
                     glm::value_ptr(glm::ortho(0.0f, spriteSize.x * 2.0f, 0.0f,
                                               spriteSize.y * 2.0f)));
...
void Sprite::render_sliced_to_texture() {
...
void Sprite::set_opacity_frames(float from, float to, float speed) {
...
void Sprite::append_opacity_frames(float from, float to, float speed) {
  for (double i = 0.0; i <= 1.0; i += speed / 60.0) {
    double x_l = std::lerp(from, to, i);
    opacities.push(x_l);
  }
